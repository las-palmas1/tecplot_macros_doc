...
Создание изображений срезов и парсинг .lay файлов
-----------------------------
...
:any:`AxisSettings`, :any:`FrameSettings`, :any:`ColormapSettings`, :any:`ExportSettings` и :any:`TicksSettings`. Значения интервалов по осям, содержащихся в keyword xlim и ylim инициализатора класса :any:`AxisSettings`, являются значенями, подставлямыми в команду скриптивого языка TecPlot, устанавливающую величину интервалов по осям. При этом они могут не совпадать со значениями интервалов на результирующей картинке, так как на их величину также влияют команды, устанавливающие размер осей по отношению к размеру фрейма и размер самого фрейма. Экземпляр класса :any:`PictureCreator` содержит два метода: :any:`run_creation` и :any:`add_to_existing_macro`. Первый из них запускает уже существующий макрос, или, если макрос пуст или его вообще нет, создает макрос и записывает в него команды. Второй - добавляет команды в существующий макрос. При этом, если макрос закаканчивается командой '$!Quit', содержимое макроса перед записью удаляется. 

При создании изображений можно пользоваться настройками, содеражащимися в .lay файле. Для их считывания необходим парсинг .lay файла, который может быть осуществлен с помощью экземпляра класса :any:`LayoutParser`.

.. literalinclude::  example\picture_creation.py

В результате выполнения скрипта создаются следующие изображения.
...
.. figure:: example\images\W_long.png
	:scale: 30 %
	:align: center

	Полученное изображение распределения вертикальной составляющей скорости в продольном сечении

Simple way
^^^^^^^^^^^^
Если не задавать все настройки, которые имеют значения по умолчанию, то объем кода получится заметно меньше. Например, можно вообще не создавать экземпляры классов :any:`FrameSettings`, :any:`TicksSettings` и :any:`Font`. Настройки, содержащиеся в них, не будут задаваться в файле макроса, а элементы изображения, за которые они отвечают, будут настраиваться TecPlot'ом автоматически.

.. literalinclude::  example\picture_creation_simple_way.py

В результате выполнения скрипта создаются следующие изображения.

.. figure:: example\images\U_long_simple_way.png
	:scale: 30 %
	:align: center

	Полученное изображение распределения продольной составляющей скорости в продольном сечении

.. figure:: example\images\W_long_simple_way.png
	:scale: 30 %
	:align: center
